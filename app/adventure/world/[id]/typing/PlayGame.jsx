...
const frasesEnIngles = [
  'In the tianguis, t here are colorful stalls. Vendors sell fruits like mangoes, papayas, and bananas. You can hear the sounds of people bargaining for the best price. There are toys, clothes, and even shoes for sale.',
...
const index = Array.from({ length: frasesEnIngles.length }, (_, i) => i + 1);
...
const PlayGame = ({ onGame, onChangeScore }) => {
  const [defaultData] = useState(
    frasesEnIngles[index[0]],
    frasesEnEspanol[index[0]]
  );
  ...
  }, [dataTyping.length, defaultData, onGame, textTyping, textTyping.position]);
  ...
  return (
    <div className='grid grid-rows-2 h-full p-14 text-center gap-3 playing'>
      <ul className='list p-14 text-left rounded-xl'>
        ...
        <div className='mb-10'>
          <li id='cosa'>{frasesEnEspanol[index[0]]}</li>
        </div>
      </ul>
